...
class ShapeDetection(Node):
    ...
    def scan_callback(self, msg):
        ...
        # left_mask = (angles > np.deg2rad(60)) & (angles < np.deg2rad(90))
        # right_mask = (angles < np.deg2rad(-60)) & (angles > np.deg2rad(-90))
    ...
    def process_left_side(self, points, window_name):
        ...
        if shape in ["Square", "Triangle"] and not self.shape_detected_left:
            ...
            self.get_logger().info("Shape Detected...Robot Stopping.")
        ...
        # cv2.waitKey(1)
        try:
            self.image_pub_left.publish(self.bridge.cv2_to_imgmsg(img, encoding="bgr8"))
        except Exception as e:
            self.get_logger().warn(f"Failed to publish left image: {e}")
    ...
    def process_right_side(self, points, window_name):
        ...
        # cv2.waitKey(1)
        try:
            self.image_pub_right.publish(self.bridge.cv2_to_imgmsg(img, encoding="bgr8"))
        except Exception as e:
            self.get_logger().warn(f"Failed to publish right image: {e}")
