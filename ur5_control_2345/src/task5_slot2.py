...
class JointServoNode(Node):
    ...
    def fruit1(self):
        ...
        upper_link_target = -2.925959499698139
        forearm_target = -0.4873156766251058
        ...
        # STATE 7 — PICK FRUIT
            ...
            if self.fruit1():
                if not self.attached and self.detached:
                    ...
                    self.state = 8  # move next only when magnet is attached
                    self.get_logger().info(f"state: {self.state}")
        ...
        # STATE 10 — DROP_FRUIT
            ...
            if self.go_to_drop_fruits():
                if self.attached and not self.detached:
                    ...
                    self.state = 10  # move next only when magnet is attached
                    self.get_logger().info(f"state: {self.state}")
