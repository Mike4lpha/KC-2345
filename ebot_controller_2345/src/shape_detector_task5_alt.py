...
class ShapeDetection(Node):
    ...
    def hardcoded_shape_from_count(self, count):  # >>> ADDED
        mapping = {
            # 1: "Square",
            1: "Triangle",
            2: "Square",
    ...
    def process_left_side(self, points, window_name):
        ...
        if self.x_ok and not self.ignore_zone3:
            ...
            if merged_lines is not None:
                ...
                pre_shape, angle = self.classify_shape(merged_lines)

                if pre_shape in ["Square", "Triangle"]:
                    shape = self.hardcoded_shape_from_count(self.plant_no)
    ...
    def publish_odom(self):
        ...
        self.get_logger().info(f"Detection published: {msg.data} and plant no: {self.plant_no}")
        self.flag = True
        self.plant_no += 1
